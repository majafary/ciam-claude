workspace "CIAM Integration Suite" "Customer Identity and Access Management platform with multi-factor authentication, device trust, and electronic signatures" {
    ...
    views {
        ...
        filtered "Containers" include "element.tag==Backend || element.tag==Frontend || element.tag==Database" {
            title "[Filtered] Authentication Flow - Core Containers"
            description "Focus on authentication-related containers only"
        }

        # External Integration View
        filtered "SystemContext" include "element.tag==External || element.type==SoftwareSystem" {
            title "[Filtered] External Integrations"
            description "Shows only external systems and their relationships"
        }
